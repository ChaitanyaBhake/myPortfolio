...
const ProjectCardsMobile = () => {
  ...
  return (
    <div className="flex flex-row gap-6 px-4 items-center overflow-x-auto w-full h-[90vh] scrollbar-hide snap-x snap-mandatory">
      {projects.map((project, index) => (
        <div
          key={project.id}
          className={`relative shrink-0 w-[85vw] h-[80vh] flex ml-3 snap-center ${
        ...
        >
          ...
          >
            {/* Title */}
            <div className="w-full h-[12%] lg:h-[21%] p-2 bg-gradient-to-tr from-white to-black bg-clip-text text-transparent lg:block lg:p-5">
              <h1 className="h-full sm:text-[29px] text-[18px] font-bold md:text-5xl lg:text-6xl xl:text-7xl">
                {project.title}
              </h1>
            </div>

            {/* Project Video and Desc */}
            <div className="w-full h-[79%] flex flex-col p-3 lg:flex-row lg:p-14 lg:pt-0 lg:gap-5 lg:items-center">
              ...
              {/* Description */}
              <div
                className="w-full h-[50%] lg:w-[50%] lg:h-auto lg:flex lg:items-center lg:mt-0 rounded-3xl overflow-y-auto mt-3 pl-5 transform-gpu will-change-transform"
                style={{ minHeight: '50%' }}
              >
